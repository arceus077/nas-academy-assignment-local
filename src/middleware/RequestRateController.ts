...
class RequestRateController {
    ...
    public checkRequestRate = async (req, res, next) => {
        try {
            console.log('checkRequestRate for ip address: ', `${req.ip}`, 'checkRequestRate called');
            const { ip } = req;
            const user = this.users.find(value => value.ip === ip);
            ...
            if (!user) { 
                ...
                if (prevDate !== currentDate || currentHour !== prevHour || minutuesDifference > 1 || (secondsDifference > 10)) {
                    ...
                    const index = this.users.findIndex(value => value.ip === ip);
...
export default RequestRateController;
